...
int main() {
    uint64_t expected_values[] = {0x56, 0x44, 0x32, 0x20, 0x0E}; // 期望的值序列
    uint64_t test_value = 0x12; // 用于位清除的值
    uint64_t loaded_value, stored_value;
    int i;

    for (i = 0; i < 4; i++) {
        // 模拟 LDCLR 操作的逻辑：按位与操作与取反
        loaded_value = atomic_load(&atomicdata);   // 原子地加载当前值
        stored_value = loaded_value & ~test_value; // 对加载的值进行按位与取反的操作
